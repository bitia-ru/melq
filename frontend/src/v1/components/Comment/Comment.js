      ...
      <div className={css(styles.container)}>
        ...
        <div className={css(styles.textBlock)}>
          ...
          <div className={css(styles.btnContainer)}>
            <div className={css(styles.btn)} onClick={() => answer(comment.id)}>Ответить</div>
            {
              (notReady(user) || !notExist(user)) && (
                <>
                  <div className={css(styles.btn)} onClick={() => this.changeHiddenProp(comment.id)}>
                    {comment.hidden ? 'Показать' : 'Скрыть'}
                  </div>
                  <div className={css(styles.btn)} onClick={() => this.removeComment(comment.id)}>
                    Удалить
                  </div>
                </>
              )
